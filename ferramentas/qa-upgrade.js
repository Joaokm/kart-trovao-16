...
test('qa-frame.html carrega os mesmos scripts do index.html',()=>{const fs=require('fs'),path=require('path'),lista=f=>[...fs.readFileSync(path.join(__dirname,f),'utf8').matchAll(/<script\s+src="(?:\.\.\/)?([^"]+)"/g)].map(m=>m[1]).join(',');assert.equal(lista('qa-frame.html'),lista('../index.html'));});
